...
def display_q_values_around_agent(agent_coords: tuple[int,int], possible_actions: list[tuple[int,int]], environment_row_count: int,environment_column_count:int, q_table: tuple[tuple[float]]) -> None:
    ...
    for action in possible_actions:
        ...
        relevant_q_value = q_table[q_table_value_index][action_index]
        
        q_value_list.append(relevant_q_value)
        
        # displaying the q_value rounded so it doesn't clutter the screen
        if abs(relevant_q_value) > 1000:
            q_value_to_display = f"{relevant_q_value:.{precision_four_or_more_digits}e}"
        elif abs(relevant_q_value) > 100:
            q_value_to_display = f"{relevant_q_value:.{precision_three_digit}f}"
        else:
...
def display_run_and_action_index(run_index: int, action_index, coords_to_display:tuple[int,int], font) -> None:
    ...
    return_message = f"{return_message_run} {return_message_action}"
